package com.chessboard.editor.xgamescenes;
...
import com.chessboard.editor.GameApp;
import com.chessboard.editor.gameworld.Borders;
import com.chessboard.editor.gameworld.FrontlineService;
import com.chessboard.editor.gameworld.ParallelCacheService;
import com.chessboard.editor.gameworld.PropertyKey;
import com.chessboard.editor.gameworld.SpawnService;
import com.chessboard.editor.gameworld.Type;
import com.chessboard.editor.gameworld.collisionhandler.BulletCollisionHandler;
import com.chessboard.editor.gameworld.collisionhandler.SmallExplodableCollisionHandler;
import com.chessboard.editor.gameworld.collisionhandler.TranslucentCollisionHandler;
import com.chessboard.editor.gameworld.entity.Launcher;
import com.chessboard.editor.gameworld.entity.accessories.Shadow;
import com.chessboard.editor.gameworld.entity.characters.Clancy;
import com.chessboard.editor.gameworld.entity.characters.ClancyComponent2;
import com.chessboard.editor.gameworld.entity.characters.ClancyViewComponent2;
import com.chessboard.editor.gameworld.entity.components.DestroyableComponent;
import com.chessboard.editor.xgamescenes.control.ClancyGame;
...
import com.chessboard.editor.gameworld.*;
...
public class RogueLikeGame implements ReplaceableGameSceneWithConcurrentService, DimensionService, SpawnService, ClancyGame, ExitService, UpdateService, FrontlineService, ParallelCacheService {
  ...
  public void asyncLoadResources(AsyncLabel label) {
    ...
    var cacheSet = cacheSet(Type.GROUND, Type.GROUND_FONT, Type.CLANCY, Type.BULLET, Type.LAUNCHER_MISSILE, Type.LAUNCHER);
  ...
  public void initPhysics(List<PhysicsWorld> physicsWorlds, XInput input) {
    ...
    for (var ammo : ammos) {
      var handler = switch (ammo) {
        case BULLET, CHINOOK_BULLET, SPACE_MARINE_BULLET -> new BulletCollisionHandler(Type.CLANCY, ammo);
        default -> new SmallExplodableCollisionHandler(Type.CLANCY, ammo);
      };
      ...
      if (airAmmos.contains(ammo)) {
        physicsWorlds.getLast().addCollisionHandler(handler.copyFor(Type.GROUND, ammo));
      }
    }

    for (var unit : units) {
      physicsWorlds.getLast().addCollisionHandler(new TranslucentCollisionHandler(Type.CLANCY, unit));
    }
  }
  ...
  public void initGame(List<GameWorld> gameWorlds, XInput input) {
    ...
    spawn(Type.LAUNCHER, getGameWidth(), FXGLMath.random(650, 750)).setOnNotActive(()-> settle(true));
  ...
  private void spawnEnemyUnit(double x, double y) {
    spawn(Type.LAUNCHER, new SpawnData(x, y));
  }
